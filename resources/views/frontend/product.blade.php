...
<main class="main">
    ...
    <div class="banner banner-cat" style="background-image: url('{{ asset('storage/app/header_image/'.$bannerheading->banner_img) }}');">
        <div class="banner-content container">
            @if (!empty($bannerheading->info))<h2 class="banner-subtitle" style="color: {{ $bannerheading->info_color }}">{!! $bannerheading->info !!}</h2>@endif
            @if (!empty($bannerheading->heading))<h1 class="banner-title" style="color: {{ $bannerheading->heading_color }}">{!! $bannerheading->heading !!}</h1>@endif
            @if (!empty($bannerheading->url))<a href="{{ $bannerheading->url }}" class="btn btn-primary">Shop Now</a>@endif
    ...
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
        <div class="container">
            <ol class="breadcrumb">
                ...
                @if(!empty($gender_slug))<li class="breadcrumb-item active" aria-current="page">{{ $gender_slug->gender }}</li>@endif
            </ol>
        </div><!-- End .container -->
    </nav>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-9">
                ...
                {{-- <div class="row row-sm" id="Ajaxproducts" style="display:none;">
                    @include('frontend.watch')
                </div> --}}
                            ...
                            <div class="widget-body">
                                <ul class="cat-list" style="overflow: auto;max-height: 200px;" id="gender_list">
                                    @foreach ($gender as $item)
                                        <li class="text-dark"><input type="checkbox" name="gender[]" @if (!empty($gender_slug)) @if($gender_slug->id == $item->id) checked @endif @endif value="{{ $item->id }}"> {{ $item->gender }}</li>
                                    @endforeach
                                </ul>
                            </div><!-- End .widget-body -->
                        </div><!-- End .collapse -->
